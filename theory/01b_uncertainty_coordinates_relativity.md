...
This series is itself an example. It was originally labeled "The Physics of Metaphysics" - a coordinate choice that placed it in a region of intellectual state space (metaphysics) where much of the dynamics (empirical prediction, formal mechanics, testable claims) did not apply. Relabeling it "Epimechanics" was a coordinate transform: the series' measurable state (its content, structure, claims) did not change, but the label now places it in a region where the dynamics are more applicable. The original label was a coordinate error - it correctly identified the series as having a state, but placed it in *the wrong state space*. The relabeling was predictable: a framework that emphasizes the importance of choosing the right $X$ should eventually apply that principle to its own name.
