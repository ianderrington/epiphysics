...
graph TD
    ...
    F --> I["`🌿 **Start Work**
    sc git smart branch + sc planning req update`"]
    G --> I
    H --> I
...
```json
{
  ...
  "recommendedActions": [
    {
      "command": "sc planning req validate 001",
...
requirement_validation = true        # sc planning req validate --all
...
- **Requirement validation**: `sc planning req validate --all` (blocks on invalid requirements)
...
#### After `sc git smart branch` + `sc planning req update --status=in-progress`
...
Suggestion: sc planning req sync
```
...
Located in `.supernal-local/workflow-state.json`:
...
- Check `.supernal-local` directory permissions
