...
- Run 'sc planning req new' to create your first requirement
```
...
sc planning req new "Role-Based Access Control" \
  --priority=high \
  --epic=authorization \
  --compliance="iso-13485" \
  --phase=discovery

sc planning req new "Audit Logging System" \
  --priority=high \
  --epic=audit \
  --compliance="iso-13485" \
  --phase=discovery

sc planning req new "Data Security & Encryption" \
  --priority=critical \
  --epic=security \
  --compliance="iso-13485" \
  --phase=discovery
...
sc planning req expand REQ-001 --detailed

# Generate test plans
sc planning req generate-tests REQ-001
```
...
sc planning req update REQ-001.1 --phase=foundation
sc planning req update REQ-001.2 --phase=foundation

# Create foundation requirements
sc planning req new "Database schema for user management" \
  --priority=high \
  --epic=infrastructure \
  --phase=foundation \
  --compliance="iso-13485"

sc planning req new "API endpoint structure and validation" \
  --priority=high \
  --epic=infrastructure \
  --phase=foundation

sc planning req new "Security middleware implementation" \
  --priority=critical \
  --epic=security \
  --phase=foundation \
  --compliance="iso-13485"
...
sc planning req update REQ-001.1 --status=in-progress
```
...
sc planning req validate REQ-001.1 --include-tests --include-compliance

# Update requirement status
sc planning req update REQ-001.1 --status=done --notes="User registration implemented with bcrypt hashing and validation"
```
...
sc planning req update REQ-001.1 --phase=validation
