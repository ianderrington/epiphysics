...
End-to-end development process from business discovery through production deployment. Organized around natural work breakdown hierarchy: **Milestone → Epic → Requirement → Implementation**.

**Note**: "Tasks" are small operational work items (1-8 hours) managed separately via `sc task` - see [SOP-T.08](./tools/SOP-T.08-using-sc-task.md). Requirements are formal specifications for user-facing features requiring acceptance criteria and testing strategies.

## Key Principles

1. **Hierarchical Breakdown**: Milestone → Epic → Requirement → Implementation
   - **Tasks**: Separate operational work items (1-8 hours), not part of requirement hierarchy
...
**Feature Development Workflow**: After Phase 4 planning, use cursor rule: feature-flow.mdc for complete implementation workflow:
```bash
# Quick workflow reference:
sc planning feature create --id={feature} --domain={domain} --epic={epic}
# → Write planning doc with **File**: pattern
# → sc docs process <doc> --commit
# → Create/run tests iteratively
...
**Location**: `docs/requirements/comp/`
...
**Step 3: Break into Tasks & Assignments**
...
6. **Create agent assignments** for multi-agent/parallel work

**Agent Assignments** (for complex features):
- Pre-work specifications for agents
- Clear scope with requirements, tests, implementation
- Enable parallel development (multiple agents simultaneously)
- Document dependencies between agents
- See: [SOP-T.10: Using Agent Assignments](tools/SOP-T.10-using-agent-assignments.md)
...
- Estimate effort
- Create assignment documents for parallel work

**Outputs**:

- Epic documents with linked features
- Feature documents with linked stories and tasks
- Task documents with specifications (`sc task new`)
- **Agent assignments** (`docs/features/{domain}/{feature}/assignments/`)
- Dependency maps
- Timeline estimates
