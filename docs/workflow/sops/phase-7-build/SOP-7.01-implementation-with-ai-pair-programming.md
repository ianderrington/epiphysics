...
| [Testing Guide](../../../guides/testing/testing-guide.md) | Test structure, naming, quality requirements |
...
When you create ANY file during development:

1. **Commit immediately** if complete and tested
   ```bash
   sc git commit <file> -m "[FEATURE:X] description"
   ```

2. **WIP register** if work in progress
   ```bash
   sc workflow wip register <file> --feature=X --requirement=REQ-XXX
   ```

3. **Ignore** if generated/temporary
   ```bash
   echo "<pattern>" >> .gitignore
   ```

**NEVER leave files untracked** - accumulating 5+ untracked files will block commits.

See cursor rule: file-creation-protocol.mdc for complete decision tree.
...
```bash
# Commit ONLY files you modified (never use git add .)
...
# Supernal git hooks run automatically and validate:
...
**Important**: Always commit only files you specifically modified during your work. Never use `git add .` or `git add -A` unless explicitly requested by the user.
...
See [Documentation Processor Feature](../../../features/developer-tooling/documentation-processor/README.md) for full details.
