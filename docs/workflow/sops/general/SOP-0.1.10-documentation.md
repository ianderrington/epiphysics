...
**See**: cursor rule: template-sync-policy.mdc for complete workflow and approval process.

**Quick Commands**:
```bash
sc docs merge-templates --dry-run  # Preview changes
sc docs merge-templates --report   # Generate approval report
sc docs merge-templates             # Execute merge
```

**Key Points**:
- Preserves `reviewedBy`, `reviewDates`, `status`, `version_history`
- Updates template content and structure
- Auto-detects when re-approval is needed
- Generates approval reports

**Related**:
- [SOP-0.1.13: Change Control](./SOP-0.1.13-change-control.md#template-sync-changes)
