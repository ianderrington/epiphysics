...
### Controlled Files Protection

**Critical safeguard**: Some files require workflow approval before AI modification

**Three control levels**:

1. **Strict** (CHG document required):
   - `supernal.yaml`, `package.json`, `.husky/**/*`
   - Core infrastructure and enforcement files
   - Requires explicit approval from designated approvers

2. **Moderate** (Requirement + tests required):
   - `supernal-code-package/cli/**/*.js`
   - `supernal-code-package/lib/**/*.js`
   - Core functionality requiring traceability

3. **Light** (Justification required):
   - `tsconfig.json`, `.eslintrc.js`, `jest.config.js`
   - Configuration files with lower risk
...
# 3. Complete prompts (CHG/REQ creation)

# 4. NOW you can modify the file

# 5. Commit will be allowed (pre-commit hook validates)
git commit -m "chore: Update (CHG-XXXXXX)"
```

**Pre-commit enforcement**:
- Hook **hard-blocks** commits if controlled file modified without workflow
- Clear error messages with fix commands
- Emergency bypass: `SC_ALLOW_CONTROLLED_EDIT=1` (audited)

**See**: cursor rule: controlled-files.mdc for complete reference
...
- [ ] **Checked if file is controlled** (`sc controlled check <file>`)
