...
### Component Test IDs (data-testid)

**Use Name Contracts for all component IDs** (never hardcoded strings):
...
export function SecurityView() {
  return (
    <div data-testid={Components.Compliance.SecurityView}>
      {/* Type-safe, refactor-safe, searchable */}
    </div>
  );
}
```
...
- ✅ Searchable (`grep "Components.Compliance"` finds all uses)
- ✅ Testable (automated validation ensures contracts match code)
- ✅ Auto-maintained (monitor daemon keeps contracts fresh)

**See**: cursor rule: name-contracts.mdc, [Feature Docs](../../../features/developer-tooling/name-contracts/README.md)

### Route Patterns

**Use Route Contracts for all route references** (never magic strings):

```typescript
import { Routes, buildRoute } from '@/lib/routing/Routes';

// In tests
await page.goto(buildRoute(Routes.Views.Overview, { repo: 'my-repo', branch: 'main' }));

// In API calls
const response = await fetch(buildRoute(Routes.API.RepoIdRequirements, { repoId: 'my-repo' }));
```

**Generate Contracts**:
```bash
# Scan and generate all contracts (components + routes)
sc code generate

# Routes only
sc code generate --type=routes
```

**Benefits**:
- ✅ Type safety (IDE autocomplete for all routes)
- ✅ Refactor safe (route changes break at compile-time)
- ✅ Auto-extracted (no manual route definitions needed)
- ✅ Testable (all routes validated automatically)

**See**: [Route Name Contracts](../../../features/developer-tooling/route-name-contracts/README.md), cursor rule: name-contracts.mdc
