...
| Pattern | Description |
|---------|-------------|
| `.supernal-local/sessions/` | Session directories |
| `.supernal-local/integrations/` | Integration credentials |
...
- Files in `.supernal-local/integrations/`
- Any `*.key` files
...
- `.supernal-local/encryption.key` exists
- `.supernal-local/key-rotation.json` exists
...
- Credential Storage Specification - Security architecture documentation (see components/credential-storage.md)
