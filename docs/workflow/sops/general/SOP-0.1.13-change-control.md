...
8. **Controlled file modifications** (enforced by pre-commit hook)
...
**Automated pre-commit enforcement** for critical infrastructure files.

**Three control levels**:

1. **Strict** - CHG document + approval required:
   - `supernal.yaml` - Core configuration
   - `package.json` - Dependencies
   - `.husky/**/*` - Git hooks
   - `supernal-code-package/lib/validation/**/*` - Enforcement logic

2. **Moderate** - Requirement + tests required:
   - `supernal-code-package/cli/**/*.js` - CLI commands
   - `supernal-code-package/lib/**/*.js` - Core library
...
```bash
# Before modifying controlled file
sc controlled modify supernal.yaml

# System prompts for:
# - CHG document creation (strict)
# - Requirement linkage (moderate)
# - Justification (light)
...
**See**: 
- cursor rule: controlled-files.mdc
- REQ-WORKFLOW-144
