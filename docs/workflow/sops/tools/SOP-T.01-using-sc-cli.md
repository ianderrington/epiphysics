...
AI: sc planning req new "User login with JWT" --epic=auth --priority=high
...
sc planning req new "Feature title" --epic=epic-name --priority=high --request-type=feature

# 2. Validate requirement
sc planning req validate REQ-XXX

# 3. Create branch if needed (optional - can work on main)
# Work session: sc session start --name=alice-dec15
# Feature branch: sc git smart branch --branch=feature/req-XXX-title

# 4. Mark in-progress and update during development
sc planning req update REQ-XXX --status=in-progress
sc planning req update REQ-XXX --prompt="Discovered X, need to handle Y"
...
AI: sc planning req new "Password reset with email verification" \
      --epic=auth \
      --priority=high \
...
# Step 3: Validate
AI: sc planning req validate 043
...
AI: sc planning req generate-tests 043
...
AI: sc planning req update 043 --status=in-progress
...
AI: sc planning req new "Fix login with special chars" \
      --epic=auth \
      --priority=critical \
      --request-type=bug
...
AI: sc planning req validate 044
AI: sc git smart branch --branch=feature/req-044-bug-fix
AI: sc planning req update 044 --status=in-progress
...
AI: sc planning req new "Implement OAuth 2.1 authorization" \
      --epic=auth \
      --priority=high \
      --request-type=feature

# Continue with normal workflow...
AI: sc planning req validate 045
AI: sc planning req generate-tests 045
AI: sc git smart branch --branch=feature/req-045-oauth
AI: sc planning req update 045 --status=in-progress
# ... etc
```
...
sc planning req new "Title" --epic=name --priority=high --request-type=feature|bug|enhancement|maintenance

# List requirements
sc planning req list

# Show specific requirement
sc planning req show REQ-XXX
...
sc planning req update REQ-XXX --status=in-progress|done|blocked

# Update with context
sc planning req update REQ-XXX --prompt="Additional context about changes"
```
...
sc session start --name=alice-dec15  # Work session (preferred)
sc git smart branch --branch=feature/my-feature  # Feature branch (optional)
...
See cursor rule: documentation-processor.mdc for full pattern details.
...
├─ Yes → sc planning req new
│         └─ sc planning req validate
│            └─ sc planning req generate-tests
│               └─ sc git smart branch
│                  └─ sc planning req update --status=in-progress
│                     └─ [implement]
│                        └─ sc git smart merge
│
...
sc planning req new "Title" --epic=X --priority=Y --request-type=Z
...
sc planning req remove REQ-XXX
...
```bash
# Attempt 1
AI: sc planning req validate 042
...
AI: sc planning req new "User login" --epic=auth --priority=high
...
```
AI: sc planning req generate-tests 042
...
AI: sc planning req update 042 --status=in-progress
...
- ✅ Always validate before starting work (`sc planning req validate`)
- ✅ Generate tests from Gherkin (`sc planning req generate-tests`)
