...
**This SOP describes the pattern now formalized as "Work Sessions".**

**Recommended Approach** (2025-12-15):
- **Work Sessions** : Personal session branches for multi-feature work
- See: `docs/features/workflow-management/work-sessions/README.md`

**This SOP remains valid** for understanding the multi-feature pattern and WIP tracking, but work sessions provide:
- Clearer naming conventions (`session-{person}-{date}`)
- Better tooling (`sc session start/finish`)
- Lifecycle management
- Multi-person coordination

**Use this SOP for**: Understanding the pattern
**Use work sessions for**: Implementation
...
feature/payment-system
  - payment-types     (example) - Define interfaces FIRST
  - payment-api       (example) - Implements contract
  - payment-ui        (example) - Uses contract
  - payment-tests     (example) - Tests contract
```
...
feature/sprint-1
  - payment-api      (example) - src/payment/
  - auth-dashboard   (example) - src/dashboard/
  - email-templates  (example) - src/email/
```
...
git commit -m "feat(scope): REQ-099: Define payment API contract
...
git commit -m "feat(scope): REQ-100: Implement payment API
...
git commit -m "feat(scope): REQ-101: Build checkout form
...
git log --grep="feat(scope):"
git log --grep="feat(scope):"

# Or use the command
sc planning features commits payment-api
sc planning features list  # Show all features
```
...
git commit -m "feat(scope): REQ-100: Add endpoint ✅ Tests passing"

# ❌ DON'T: Commit untested code
git commit -m "feat(scope): REQ-100: Add endpoint (untested)"
```

### 5. **Feature Tagging for Clarity**

Every commit tagged with `feat(scope):`:
...
sc planning features add payment-types --requirements=REQ-099
# ... write types.ts, commit immediately

# Now work on everything in parallel
sc planning features add payment-api --requirements=REQ-100
sc planning features add payment-ui --requirements=REQ-101
sc planning features add payment-tests --requirements=REQ-102

# All three developed simultaneously - no blocking!
git commit -m "feat(scope): REQ-099: Define contract ✅"
git commit -m "feat(scope): REQ-100: Implement API ✅ Tests passing"
git commit -m "feat(scope): REQ-101: Build UI ✅ Tests passing"
git commit -m "feat(scope): REQ-102: E2E tests ✅ Tests passing"
...
sc planning features add payment-api --requirements=REQ-100
sc planning features add dashboard-metrics --requirements=REQ-101
sc planning features add email-notifications --requirements=REQ-102

# Work on all three in parallel
git commit -m "feat(scope): REQ-100: Add endpoint ✅ Tests passing"
git commit -m "feat(scope): REQ-101: Add charts ✅ Tests passing"
git commit -m "feat(scope): REQ-102: Add templates ✅ Tests passing"
...
sc planning features add payment-api
sc planning features add payment-ui  # Blocks! Doesn't know API format
...
sc workflow wip register <file> --feature=<feature-name> --requirement=REQ-XXX
```
...
git commit -m "feat(scope): REQ-XXX: Description"
```
...
- cursor rule: wip-registry.mdc - WIP tracking system
