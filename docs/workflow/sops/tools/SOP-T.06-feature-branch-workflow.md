...
**This SOP describes traditional feature branches (one branch per feature).**

**Workflow Options** (see SOP-0.1.12 for full comparison):

1. **Trunk-Based** (Default): Work on main with `feat(scope):` commits
2. **Work Sessions** (Preferred when branches needed): Personal branches for multi-feature work
3. **Feature Branches** (This SOP): One branch per feature (good for PRs, external contributors)

**Use feature branches when**:
- External contributors submitting PRs
- Strict code review requirements
- Need isolated feature development
- Open source or public repositories

**Consider work sessions instead when**:
...
```bash
sc planning kanban new "Feature: User Dashboard" \
  --type=proposal \
  --epic=epic-dashboard \
  --priority=high \
  --assignee=@username
...
sc planning feature move feature-user-dashboard --phase=implementing
# Updates frontmatter: phase: drafting → phase: implementing

# Move to testing
sc planning feature move feature-user-dashboard --phase=testing
# Updates frontmatter: phase: implementing → phase: testing

# Move to validating
sc planning feature move feature-user-dashboard --phase=validating
...
```bash
# Update with notes
sc planning kanban update feature-user-dashboard \
...
# Update kanban
sc planning kanban complete feature-user-dashboard
...
sc planning feature move feature-user-dashboard complete
...
```bash
sc planning kanban archive feature-user-dashboard
...
sc git branch switch main

# Switch to feature branch
sc git branch switch feature/feature-user-dashboard
...
# Features on main
sc planning feature audit --branch=main

# All features across branches
sc planning feature audit --all-branches
...
sc planning feature move <name> <phase>

# Signal ready to merge
sc feature ready-to-merge <name>

# List features
sc planning feature audit [--branch=<name>] [--all-branches]
```
...
sc git branch switch <branch-name>

# Compare branches
sc git branch diff [<branch1>..<branch2>]
```
...
```bash
# Create proposal
sc planning kanban new "<title>" --type=proposal

# Approve proposal
sc planning kanban approve <card-id>

# Update progress
sc planning kanban update <card-id> --note="<progress>"

# Complete (after merge)
sc planning kanban complete <card-id>

# Archive
sc planning kanban archive <card-id>

# List kanban items
sc planning kanban list [--branch=<name>] [--status=<status>]
```
