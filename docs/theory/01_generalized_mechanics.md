...
**Note:** $\rho_{\text{ac}}(x)$ is evaluated at point $x$ but reflects the loop structure passing through $x$ — it is an emergent property of causal cycles, not measurable from single-point bond data alone (see Part 1.5 on emergent auto-causality).
