...
The formal system constructed in this series is called **Epimechanics** (Greek ἐπί, 'upon' + mechanics) - the mechanics that sits upon domain-specific mechanics. **EpiMech** for short.

What follows is the full Epimechanics framework, one concept at a time.
...
This spectrum connects directly to life-likeness ([Part 1c](./epimechanics_01c_thermodynamic_emergence_of_life.md)). Epimechanics predicts that higher $\rho_{\text{ac}}$ corresponds to: greater resistance to perturbation (the entity restores itself after disruption), longer persistence ($T_{\text{local}}$ is larger), stronger causal influence on other systems ($\Pi_{\text{out}}$ is higher), and - when combined with informational coupling ($I > 0$) and hereditary transmission ($\mathbf{R} > 0$) - the characteristics we associate with life. The emergence of life is, in these terms, the emergence of entities with very high $\rho_{\text{ac}}$ sustained by thermodynamic energy throughput and informational self-organization.

> **Forward references.** The quantities $T_{\text{local}}$ (persistence duration) and $\mathbf{R}$ (representational footprint) are formally defined in [Part 4](./epimechanics_04_time_and_soul.md) (Time and Soul); $\Pi_{\text{out}}$ (outward causal power) and $I$ (informational coupling / intelligence) are formally defined in [Part 3](./epimechanics_03_intelligence_consciousness_agency.md) (Intelligence, Consciousness, and Agency). They are previewed here to motivate the entity spectrum; full definitions are not required until those later parts.
...
The question of when a *collection* of entities develops its own auto-causal density - when the aggregate sustains itself through its own internal loops rather than through the loops of its parts - is the meta-entity question, addressed in [Part 2](./epimechanics_02_meta_entities.md).
...
This ordinal prediction is weaker than full equivalence but not trivial - it can fail (structurally complex but dynamically fragile entities would break the unified mass concept). The long-term goal is cardinal agreement with domain-specific units. [Part 5, Section 4.0](./epimechanics_05_ontology_and_open_questions.md) develops the full discussion of what Epimechanics predicts, what it does not, and where the tautology boundaries lie.
...
> [!sidenote]
> *Note on the scalar approximation and the mass tensor.* The form $p = \mathcal{M} \cdot \dot{X}$ treats mass as a scalar - the same resistance to change in every direction through state space. This is valid when the state space is **isotropic** with respect to the entity: the entity resists change equally in all dimensions. For physical particles in Euclidean space, this holds by symmetry. For abstract entities, it generally does not. A belief system resists identity-related change far more than it resists aesthetic change. An institution resists changes to its core mission more than changes to its branding. The resistance is direction-dependent - which means mass is properly a **tensor**, not a scalar.
>
> The general form is:
>
...
>
> The mass tensor $\mathcal{M}_{ij}$ handles **within-entity anisotropy** - different resistance in different directions for the same entity. This is distinct from the coupling tensor $T^i{}_j$ (Section 5b), which handles **cross-domain propagation** - how a force in one domain produces a response in another. They are different tensors serving different purposes: $\mathcal{M}_{ij}$ acts on the entity's own velocity to produce momentum; $T^i{}_j$ acts on external field forces to produce the force experienced by the entity. In the fully general case, both are needed simultaneously.
>
> The scalar approximation $p = \mathcal{M} \cdot \dot{X}$ is used throughout this series because it captures the essential physics - mass as resistance to change - without the notational overhead of indices. All equations in the series should be understood as the isotropic special case; the tensorial generalization follows by the substitution $\mathcal{M} \to \mathcal{M}_{ij}$ with appropriate index contraction.
...
> **Metric caveat.** The norm $\lvert\dot{X}\rvert^2 \equiv g_{ij}\,\dot{X}^i\dot{X}^j$ presupposes a metric $g_{ij}$ on the tangent space of $S$. For physical position in Euclidean space this is the standard dot product. For abstract state spaces - beliefs, institutions, ecological niches - the metric is a non-trivial structural choice that determines what "distance" and "speed" mean in that domain; it carries empirical content and must be specified domain by domain. See [Part 5, Open Question 2](./epimechanics_05_ontology_and_open_questions.md#open-question-2-the-geometry-of-state-space) for further discussion. A person rapidly revising beliefs has high kinetic energy in the epistemic dimension. A market in free fall has high kinetic energy in the price dimension.
>
...
> [!sidenote]
> *Why the $c^2$ form may survive coarse-graining*: All higher-level phenomena - biological, psychological, social - are implemented in physical substrate. The physical substrate has Lorentz invariance. When you coarse-grain from fundamental physics upward (quarks → atoms → cells → organisms → institutions), you integrate out fast degrees of freedom at each scale, producing an effective theory for the slow ones. The key structural question is whether the pseudo-Riemannian metric signature survives this coarse-graining.
>
> It should, for a topological reason: at every scale, there exists a maximum speed at which causal influence propagates. Neural signals: ~100 m/s. Social information via modern media: bounded by human processing speed at ~seconds per decision. Institutional change: bounded by decision cycles at ~days. Each domain has its own effective $c_{\mathcal{D}}$ - the bottleneck speed of the causal chain at that scale - and all are bounded from above by $c$. This maximum speed defines an effective light cone at each scale: causally connected events (inside) vs. causally disconnected events (outside). A policy change in Washington cannot influence Tokyo's response faster than the communication channel allows. This IS a light-cone structure, and it IS pseudo-Riemannian.
>
> If the coarse-graining preserves this structure - and it should, because causal ordering is topological and topological properties tend to survive coarse-graining - then the 4-vector structure that produces $E = mc^2$ also survives at each effective scale: $E_{\text{int},\ell} = \mathcal{M}_\ell \cdot c_{\mathcal{D},\ell}^2$, where $\mathcal{M}_\ell$ is the effective mass at scale $\ell$ and $c_{\mathcal{D},\ell}$ is the effective maximum propagation speed. The $c^2$ form is then not an analogy imported from physics but a structural invariant inherited through renormalization group flow. Whether this argument is rigorous - whether coarse-graining *always* preserves pseudo-Riemannian structure or can break it at certain phase transitions - is an open question connecting to the Ruliad program ([Part 5, Section 2](./epimechanics_05_ontology_and_open_questions.md)).
...
> [!caveat] **Equilibrium limitation.** This equipartition definition of $\mathcal{T}$ is strictly valid at or near thermodynamic equilibrium. Most interesting social, psychological, and institutional systems are far from equilibrium - driven, dissipative, and subject to external forcing. For such systems, $\mathcal{T}$ can still be defined operationally as the average kinetic energy per degree of freedom, but the full thermodynamic framework (free energy, entropy, phase transitions) requires care: the equilibrium relations among $\mathcal{T}$, $S_{\text{ent}}$, and $\mathcal{F}$ may not all hold simultaneously. This is an acknowledged limitation. Non-equilibrium statistical mechanics - particularly the Jarzynski equality and fluctuation theorems - provides rigorous extensions that Epimechanics could incorporate but has not yet developed.
...
[→ Part 1b: Uncertainty, Coordinates, and Relativity](./epimechanics_01b_uncertainty_coordinates_relativity.md) | [→ Part 2: Meta-Entities](./epimechanics_02_meta_entities.md)
