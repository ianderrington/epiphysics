...
A **representation** $X$ is a model of state — the map. It can model actual state, potential state, or any state. Representations can take many forms: point estimates, probability distributions, partial observations, compressed encodings. All are partial projections of the territory. $X$ evolves over time as state changes and as observations accumulate.
...
[Hoffman's Interface Theory (*The Case Against Reality*, 2019)](https://wwnorton.com/books/9780393254693) makes the sharpest version of this point: our perceptions are fitness-tuned interfaces, not accurate depictions - the desktop icon does not resemble the magnetic patterns on the disk. $X$ has the same status. Some representations track real causal structure well enough to predict dynamics; others are arbitrary or misleading. **Representational fidelity** $\mathcal{F}$ measures how well $X$ tracks its target state. Given sufficient computation, predictive accuracy scales with fidelity. Epimechanics' claim is that when $X$ is chosen to track real structure, the mechanical relationships derived from it (force, energy, coupling) predict how that structure evolves.

This is a methodological commitment, not a metaphysical one. We do not claim that reality is "made of" states. We claim that for any system, you can construct a representation $X$ of state within the potential state space $\mathcal{X}$, and the system's behavior may be described by how that representation evolves over time. [Quantum indefiniteness](https://plato.stanford.edu/entries/qt-issues/), [relational ontology](https://plato.stanford.edu/entries/structural-realism/), and [vagueness](https://plato.stanford.edu/entries/vagueness/) all challenge the idea that everything has a unique, definite state. Epimechanics accommodates these naturally: $X$ is already a distribution, representations can be defined relative to an observer, and the precision of $X$ is chosen to match the application. Epimechanics requires that representations exist - not that they be unique, not that they be accurate, not that they be metaphysically fundamental, and not that any particular coordinate choice be privileged over another. Like coordinates in physics, some choices of $X$ reveal more structure than others - but the underlying reality exists regardless of how well or poorly we represent it.
...
| [Part 0b: Event Layer](./00b_event_layer.md) | The foundation beneath physics | The causal event primitive; the cause-plex; three properties (P1–P3) from which spacetime, energy, and quantum mechanics emerge; no physics assumed |
...
| [Part 1.5: Causors](./01_5_causors.md) | What are entities made of? | Structure Layer: bonds and loops; Descriptor Layer: Q1–Q5 (energy mode, output target, topology, leverage, timescale); auto-causality is a loop-level emergent property — **requires Part 0b, Part 1** |
...
![Concept flow across the six parts of the series, showing how ideas build from mechanics through meta-entities, mind, time and soul, to synthesis](./images/concept_flow.svg)
