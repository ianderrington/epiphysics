...
A **representation** $X$ is a model of actual state within $\mathcal{X}$ — the map. Representations can take many forms: point estimates, probability distributions, partial observations, compressed encodings. All are partial projections of the territory. $X$ evolves over time as the actual state changes and as observations accumulate.
...
| [Part 1.5: Causors](./01_5_causors.md) | What are entities made of? | Two Structure Layer structures (bond operator, loop operator) with Q1–Q4 descriptors (energy mode, output target, topology, leverage); derived Observable Layer quantities include generalized mass, auto-causal density, maintenance cost; auto-causality is a loop-level emergent property — **requires Part 1** |
