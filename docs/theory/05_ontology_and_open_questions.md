...
**Reality**: That which exists independently of any description. Reality is not X - X is a representation, a formal description of some aspect of reality's condition. The manifold of all states X across all domains - physical, biological, informational, social, conceptual - is our representational apparatus, not reality itself. There is one reality; what differs across disciplines is the projection of our representational framework onto it. The physical sciences project onto matter and energy. The social sciences project onto agents and institutions. Epimechanics is substrate-neutral and covers all projections simultaneously. All representations have epimechanical structure (this is a mathematical triviality - see "The Universality of Epimechanical Structure" below). The claim is that representations which track real causal structure have *simple* epimechanical structure - simple enough to predict dynamics at low computational cost.

---

**X (State Variable)**: A representation - a formal description of some measurable or definable aspect of a system's condition. $X \in S$, where $S$ is the state space of possible representations. Physical position is one instance. Belief intensity, organizational cohesion, cultural salience, psychological valence - all are instances. X is not the system; it is our model of the system's condition. The generality of X is a key move of Epimechanics: X is not committed to any substrate. All representations have epimechanical structure; what distinguishes good representations is that their epimechanical structure is simple and predictive.
...
**$L$ (Lagrangian) and the Action Principle**: The Lagrangian $L = T - V = \frac{1}{2}\mathcal{M}\lvert\dot{X}\rvert^2 - V(X)$ (scalar approximation; general: $L = \frac{1}{2}\mathcal{M}_{ij}\dot{X}^i\dot{X}^j - V(X)$) is kinetic minus potential energy. The **principle of stationary action** states that the actual trajectory $X(t)$ is the one that makes the action $S = \int L \, dt$ stationary: $\delta S = 0$. This is the most fundamental formulation of the mechanics - more fundamental than the force equation, which is derived from it via the Euler-Lagrange equation. The Lagrangian defines conjugate momentum ($p = \partial L / \partial \dot{X}$), derives the force equation ($F = \mathcal{M}\ddot{X} + \dot{\mathcal{M}}\dot{X}$), and via Noether's theorem connects symmetries to conservation laws: time-translation symmetry gives energy conservation; state-space translation symmetry gives momentum conservation. Equilibrium principles complement the dynamics: minimum energy at constant entropy, minimum free energy at constant temperature ([Friston's free energy principle](https://doi.org/10.1038/nrn2787) is the biological case), and maximum entropy production for far-from-equilibrium dissipative structures.
...
This means: the state space S in which any X lives is a projection of the Ruliad. Epimechanics, which is substrate-neutral over any state space S, is therefore operating at the same level of abstraction as the Ruliad itself. It is the same mathematics at a level of abstraction that includes Wolfram's fundamental framework as a special case.
...
The Lagrangian formulation ([Part 1, Section 4b](./epimechanics_01_generalized_mechanics.md)) requires $S$ to be a smooth manifold - otherwise derivatives like $\partial L / \partial \dot{X}$ do not exist. Given this assumption, the coupling $T^i{}_j$ is a genuine $(1,1)$-tensor on $S$: a coordinate-free bilinear map $T: T_X^* S \to T_X S$ ([Part 1, Section 5b](./epimechanics_01_generalized_mechanics.md)). The notation $T^i{}_j$ is its representation in a particular coordinate chart, and domain labels like "economic" and "psychological" are coordinate choices, not fundamental structure.
...
- **How does Friston's free energy principle relate to the Lagrangian?** [Part 1](./epimechanics_01_generalized_mechanics.md) notes that Friston's variational free energy plays the role of $V(X)$ in the Euler-Lagrange equation, and that active inference is the dynamics of stationary action. Formalizing this connection - showing that Friston's framework is the biological special case of the generalized Lagrangian - would unify the free energy principle with the generalized mechanics at the level of derivation rather than analogy.
- **Does the $c^2$ form of internal energy survive coarse-graining?** In physics, $E_{\text{int}} = \mathcal{M}c^2$ is derived from the pseudo-Riemannian metric of spacetime. Epimechanics proposes $E_{\text{int}} = \mathcal{M}c_{\mathcal{D}}^2$ for general domains, where $c_{\mathcal{D}}$ is the effective maximum causal propagation speed at each scale. The argument ([Part 1, Section 4](./epimechanics_01_generalized_mechanics.md)) is that coarse-graining from fundamental physics preserves the pseudo-Riemannian structure (light-cone / maximum propagation speed) at each effective scale, so the $c^2$ form is inherited through renormalization group flow rather than imported by analogy. Whether this is rigorous - whether coarse-graining *always* preserves pseudo-Riemannian structure, or can break it at phase transitions between scales - is an open question. If the metric signature changes under coarse-graining, $E_{\text{int}}$ would take a different functional form at that scale.
...
Let $\mathcal{F}$ denote Epimechanics: the complete set of definitions, structural arguments, and formal relations assembled in [Parts 1](./epimechanics_01_generalized_mechanics.md) through 5. Let $X_{\mathcal{F}}$ be its state - the content, coherence, scope, and adoption of Epimechanics at any moment.
...
[← Part 4: Local Time, Non-Local Time, and Soul](./epimechanics_04_time_and_soul.md)
