...
```bash
sc planning req new "User authentication system" --priority=high --epic=auth
```

### 5. Start Development

```bash
sc git smart branch --branch=feature/req-001-auth  # Creates feature branch
sc planning req update REQ-001 --status=in-progress         # Mark as in-progress
...
```
my-compliant-app/
├── .supernal-local/           # Supernal Coding configuration
...
└── supernal.yaml              # Supernal YAML configuration
```
...
1. **Creation**: `sc planning req new "Feature description"`
2. **Planning**: `sc planning req expand REQ-001` (break into subtasks)
3. **Development**: `sc git smart branch` + `sc planning req update --status=in-progress`
4. **Testing**: `sc test validate REQ-001`
5. **Completion**: `sc planning req update REQ-001 --status=done`
...
```bash
sc planning req new "User login with email and password" \
  --priority=high \
  --epic=authentication \
...
```bash
sc planning req show REQ-001           # View requirement details
sc planning req expand REQ-001         # Break into subtasks
sc planning req generate-tests REQ-001 # Create test framework
```

### 3. Start Development

```bash
sc git smart branch --branch=feature/req-001-user-login  # Create feature branch
sc planning req update REQ-001 --status=in-progress               # Mark as in-progress
```
...
sc planning req validate REQ-001       # Validate requirement completion
```
...
```bash
sc planning req list                   # List all requirements
sc planning req show REQ-001          # Show requirement details
sc planning req new "Description"     # Create new requirement
sc planning req update REQ-001 --status=done
```
...
Ready to build compliant, AI-maintained software? Let's dive deeper into the [CLI Commands](../cli-commands/index.md) or explore the [Workflow Guide](./index.md).
