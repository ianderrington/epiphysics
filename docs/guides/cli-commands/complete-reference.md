...
Start --> Existing{"📁 Existing Project?"}
    ...
    ReqFlow --> ReqNew["sc planning req new 'Feature'"]
    ReqFlow --> ReqList["sc planning req list"]
    ReqFlow --> ReqValidate["sc planning req validate REQ-001"]
    ReqFlow --> ReqStart["sc git smart branch + sc planning req update"]
...
chart={`
stateDiagram-v2
[*] --> Draft : sc planning req new
Draft --> InProgress : sc planning req update --status=in-progress
InProgress --> Testing : Code complete
Testing --> Review : sc planning req validate
...
```bash
sc planning req new "requirement description" [options]
```
...
```bash
# Basic requirement
sc planning req new "User authentication system"

# High priority with epic
sc planning req new "Login with OAuth" --priority=high --epic=auth

# With compliance mapping
sc planning req new "Data encryption" --compliance="iso-13485,fda-21-cfr"

# Complete specification
sc planning req new "User profile management" \
  --priority=medium \
...
sc planning req list --priority=high

# Requirements in progress
sc planning req list --status=in-progress

# Authentication epic requirements
sc planning req list --epic=auth

# JSON output for automation
sc planning req list --format=json --limit=10
```
...
```bash
sc planning req show <req-id> [options]
```
...
sc planning req show REQ-001

# Detailed view with tests
sc planning req show REQ-001 --include-tests

# JSON format for automation
sc planning req show REQ-001 --format=json

# Complete information
sc planning req show REQ-001 --include-tests --include-commits --include-compliance
```
...
```bash
sc planning req update <req-id> [options]
```
...
```bash
# Mark as done
sc planning req update REQ-001 --status=done

# Change priority and assignee
sc planning req update REQ-001 --priority=critical --assignee="jane.smith"

# Add implementation notes
sc planning req update REQ-001 --notes="Implemented using JWT tokens"

# Add compliance framework
sc planning req update REQ-001 --add-compliance="sox,pci-dss"
```

### `sc planning req start-work` ⚠️ DEPRECATED
...
sc planning req update REQ-001 --status=in-progress
```
...
```bash
sc planning req validate <req-id> [options]
```
...
```bash
# Basic validation
sc planning req validate REQ-001

# Complete validation
sc planning req validate REQ-001 --include-tests --include-docs --include-compliance

# Strict mode with auto-fix
sc planning req validate REQ-001 --strict --fix
```
...
- `.supernal-local/config.json` - Project-specific settings
