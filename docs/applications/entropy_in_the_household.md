...
The [atomic decomposition](../theory/01_5_causors.md) introduces entropy production rate $\dot{S}_{\text{int}}$ — the rate at which a structure generates internal disorder. For a household, this depends on:
...
- **Entropy production rate** $\dot{S}_{\text{int}}$ and **repair rate** $\dot{R}_{\text{repair}}$ ([Atomic Decomposition](../theory/01_5_causors.md))
