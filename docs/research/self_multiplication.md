...
This may connect to **loop order** in [Part 0.5](../theory/00_atomic_structure.md): the number of levels of self-referential causal structure within an entity. A rank-$k$ transition tensor permits loop structures of depth $k-1$.
...
6. **Relationship to tensor rank and loop order.** The proposed connection between tensor rank and loop depth ([Part 0.5](../theory/00_atomic_structure.md)) is suggestive but not formalized.
